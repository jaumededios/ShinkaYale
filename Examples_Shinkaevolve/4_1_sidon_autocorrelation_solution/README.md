# 4.1 &mdash; Autocorrelation and Sidon sets

This problem is a sibling of [problem 4.2](../4_2_third_autocorrelation_solution).
...
[Problem 4.2](../4_2_third_autocorrelation_solution) minimises the same
...
is to find better constructions — structured, symmetric, refined with a
smooth optimiser — that push the ratio below $\pi/2$ and ideally down
towards the Matolcsi–Vinuesa region.
...
- **What tends to work.** Good constructions are usually supported on a
  strictly smaller sub-interval than the full width $\tfrac{1}{2}$,
  roughly symmetric around their centre, with characteristic "horns"
  near the boundary of their support (reminiscent of Sidon-set
  densities concentrating at the endpoints of $\{1, \dots, N\}$).
  Gradient descent through a smoothed $\|\cdot\|_\infty$ surrogate
  (e.g. $\log\text{-}\sum\text{-}\exp$) works well as a polishing step
  after a structured initialisation.
